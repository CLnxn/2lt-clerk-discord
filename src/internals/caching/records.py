...
class Record():
    def __init__(self, method: InternalMethodTypes, owner_id, data: Query, type= InternalTypes.USERS) -> None:
        self.method = method # update, set, delete, add
        self.data: Query = data # {tablename: {col:data, col2:data, ...}}
        self.id_map= {type:owner_id} # same as user_id or can also be guild_id for guild level storage
        self.rtype = type
